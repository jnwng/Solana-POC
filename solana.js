...
async function createMintAccount(connection, keypair) {
  const decimals = 8;
  const publicKey = await splToken.createMint(
    connection,
    keypair,
    keypair.publicKey,
    keypair.publicKey,
  ...
  console.log(
    `Token Mint Account: https://explorer.solana.com/address/${publicKey.toBase58()}?cluster=devnet`
  );
  return publicKey;
}
...
async function createTokenAccount(connection, mint, keypair) {
  try {
    const account_address = await splToken.createAssociatedTokenAccount(
      connection,
      keypair,
      mint,
      keypair.publicKey
    ...
    console.log(
      `Token Account: https://explorer.solana.com/address/${account_address}?cluster=devnet`
    );
